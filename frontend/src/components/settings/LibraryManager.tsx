...
export default function LibraryManager(props: Props) {
  ...
  return <section className="space-y-4">
    <header className="flex flex-wrap items-center justify-between gap-3">
      <h2 className="type-section-title text-text-primary">教材与分类</h2>
          ...
          {subjects.map((item, index) => {
            ...
            return <div key={`${item.name}-${index}`}>
              <div className={`flex items-center rounded-lg ${active ? 'bg-[var(--accent-soft)] text-accent' : 'hover:bg-bg-card'}`}>
                ...
                <button onClick={() => select(index, null)} className="flex min-w-0 flex-1 items-center gap-2 py-2 pr-2 text-left type-control"><FolderOpen className="h-4 w-4" /><span className="truncate">{item.name}</span><span className="ml-auto type-caption text-text-secondary">{count}</span></button>
              </div>
              {open && <div className="ml-5 mt-1 space-y-1 border-l border-border pl-2">
                {item.children.map((childName, childIndex) => <button key={`${childName}-${childIndex}`} onClick={() => select(index, childIndex)} className={`flex w-full items-center rounded-lg px-3 py-2 text-left type-control ${selectedSubjectIndex === index && selectedChildIndex === childIndex ? 'bg-[var(--accent-soft)] text-accent' : 'text-text-secondary hover:bg-bg-card hover:text-text-primary'}`}><span className="min-w-0 flex-1 truncate">{childName}</span><span className="type-caption">{activeBooks.filter((book) => belongsTo(book, item.name, childName)).length}</span></button>)}
                <button onClick={() => { select(index, null); props.onAddChild(index); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 type-control text-text-secondary hover:bg-bg-card hover:text-accent"><Plus className="h-4 w-4" />添加科目</button>
              </div>}
            </div>;
          })}
          <button onClick={() => select(-1, null)} className={`mt-2 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left type-control ${selectedSubjectIndex < 0 ? 'bg-[var(--accent-soft)] text-accent' : 'text-text-secondary hover:bg-bg-card hover:text-text-primary'}`}><Archive className="h-4 w-4" /><span className="flex-1">未分类</span><span className="type-caption">{activeBooks.filter((book) => !(book.subject || '').trim()).length}</span></button>
        </div>
      </aside>

      <main className="min-w-0 p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3 border-b border-border pb-4">
          <div><h3 className="text-lg font-semibold">{child || subject?.name || '未分类教材'}</h3><div className="mt-1 type-body text-text-secondary">{currentBooks.length} 本教材{child && subject ? ` · ${subject.name} / ${child}` : ''}</div></div>
          <button onClick={props.onRefresh} className="app-secondary-button h-9 min-h-9 px-3"><RefreshCw className="h-4 w-4" />刷新</button>
        </div>

        {subject && <div className="mt-4 border-b border-border pb-4">
          <div className="flex flex-wrap items-end gap-3">
            <label className="min-w-[220px] flex-1 type-control text-text-secondary">{child ? '科目名称' : '学科名称'}<input value={child || subject.name} disabled={selectedHasBooks} onChange={(event) => selectedChildIndex === null ? props.onRenameSubject(selectedSubjectIndex, event.target.value) : props.onRenameChild(selectedChildIndex, event.target.value)} className="mt-2 w-full rounded-lg border border-border bg-bg-card px-3 py-2 type-control text-text-primary disabled:cursor-not-allowed disabled:bg-bg-primary disabled:text-text-secondary" /></label>
            <button disabled={selectedHasBooks} onClick={() => selectedChildIndex === null ? props.onDeleteSubject(selectedSubjectIndex) : props.onDeleteChild(selectedChildIndex)} className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--danger-border)] px-3 py-2 type-control text-[var(--danger)] disabled:cursor-not-allowed disabled:opacity-40"><Trash2 className="h-4 w-4" />删除分类</button>
          </div>
          {selectedHasBooks && <p className="mt-2 type-body text-text-secondary">请先移出该分类中的教材，再重命名或删除。</p>}
        </div>}
        ...
        {!!archivedBooks.length && <section className="mt-6 border-t border-border pt-4">
          ...
          <div className="mt-3 space-y-2">
            {archivedBooks.map((book) => <div key={book.book_id || book.name} className="flex items-center gap-3 border-b border-border px-1 py-3 last:border-b-0"><div className="min-w-0 flex-1"><div className="truncate type-control">{book.display_name || book.name}</div><div className="mt-1 truncate type-caption text-text-secondary">存储名：{book.storage_name || book.name}</div></div><button onClick={() => props.onRestoreBook(book.book_id || book.name)} className="rounded-lg border border-border px-3 py-2 type-control hover:border-accent">恢复</button></div>)}
          </div>
        </section>}
      </main>
    </div>
...
function BookRow({ book, active, subjects, onMove, onSetRole, onSetResourceGroup, onSwitch, onRename, onArchive }: { book: LibraryBook; active: boolean; subjects: LibrarySubject[]; onMove: (target: string) => void; onSetRole: (role: 'standalone' | 'core' | 'reference') => void; onSetResourceGroup: (group: string) => void; onSwitch: () => void; onRename: () => void; onArchive: () => void }) {
  ...
  const assignmentOptions = useMemo(() => [
    ...
    ...subjects.flatMap((subject) => [
      { value: subject.name, label: `${subject.name}（未细分）`, group: subject.name },
      ...subject.children.map((child) => ({ value: `${subject.name}/${child}`, label: child, group: subject.name })),
    ]),
  ], [subjects]);
  return (
    <article className="grid gap-4 border-b border-border px-1 py-4 last:border-b-0 2xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
      <div className="min-w-0">
        <div className="flex min-w-0 items-center gap-2">
          <BookOpen className="h-4 w-4 flex-shrink-0 text-accent" />
          <h4 className="min-w-0 truncate type-control font-semibold text-text-primary">{book.display_name || book.name}</h4>
          {active && <span className="flex-shrink-0 rounded border border-accent/30 px-1.5 py-0.5 type-caption text-accent">当前</span>}
        </div>
        <p className="mt-1 pl-6 type-body text-text-secondary">
          ...
          <div className="mt-1.5 inline-flex rounded-lg border border-border bg-bg-card p-0.5" role="group" aria-label={`${book.display_name || book.name}的教材角色`}>
            {ROLE_OPTIONS.map((option) => (
              <button
                ...
                className={`h-9 rounded px-3 type-control ${role === option.value ? 'bg-[var(--accent-soft)] text-accent' : 'text-text-secondary hover:bg-bg-secondary hover:text-text-primary'}`}
              >
                {option.label}
              </button>
            ))}
          </div>
          <p className="mt-2 max-w-xl type-body text-text-secondary">{roleGuidance({ ...book, book_role: role })}</p>
        </div>
      </div>
      <div className="flex min-w-0 flex-col justify-between gap-4">
        <div className={`grid gap-3 ${role === 'standalone' ? 'grid-cols-1' : 'sm:grid-cols-2'}`}>
          <div>
            <div className="type-control text-text-secondary">归属</div>
            <ScrollableSelect ariaLabel={`${book.display_name || book.name}教材归属`} value={book.subject || ''} options={assignmentOptions} onChange={onMove} className="mt-1.5" />
          </div>
          {role !== 'standalone' && (
            <label className="type-control text-text-secondary">
              资料组
              <input
                ...
                className="mt-1.5 block h-9 w-full rounded-lg border border-border bg-bg-card px-2.5 type-control text-text-primary outline-none focus:border-accent"
              />
            </label>
          )}
        </div>
    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="app-popover-enter absolute bottom-10 right-0 z-30 w-36 rounded-lg border border-border bg-bg-card p-1 shadow-md" role="menu">
          <button type="button" role="menuitem" onClick={() => { setOpen(false); onRename(); }} className="flex h-9 w-full items-center gap-2 rounded px-2.5 text-left type-control text-text-primary hover:bg-bg-secondary">
            <Pencil className="h-3.5 w-3.5" />重命名
          </button>
          <button type="button" role="menuitem" onClick={() => { setOpen(false); onArchive(); }} className="flex h-9 w-full items-center gap-2 rounded px-2.5 text-left type-control text-[var(--danger)] hover:bg-[var(--danger-bg)]">
