...
- Texa had no configured MinerU API or CLI. A project-local Python 3.12 MinerU `pipeline` environment was started only after explicit approval, with package/model caches redirected to the D-drive workspace and no changes to Texa `venv310`. The user then paused the local OCR route before installation finished and before any model download, OCR page, Canonical IR, book metadata, Chroma collection, lexical snapshot, or active map mutation.
- The installation process was stopped. At the user's subsequent request, the incomplete isolated environment (`.tools/mineru312`, 203,238,892 bytes) and pip cache (`.cache/pip-mineru`, 288,875,080 bytes) were deleted after validating both resolved targets were ordinary directories inside the repository workspace and contained no reparse points. These generated files were not tracked and are recoverable only by downloading/reinstalling them. Ordered item 4 is explicitly paused rather than reported complete; item 5 has not started.

# 2026-09-15 - Textbook Quality Expansion (Professional Books Complete, Math Source Pending)

- Rebuilt and activated four production textbooks through the staged Canonical IR pipeline without editing a legacy `_chapter_map.json` in place: `误差理论与数据处理` schema 6 `ea7980a3af273f3b` (2,557 chunks / 7 chapter collections), `传感器长书` schema 6 `519dcc35e6163053` (2,204 retrievable / 2,712 catalog chunks / 14 collections), `传感器短书` schema 6 `4485780f0107cc96` (661 chunks / 13 collections), and `优化设计` schema 6 `51d33b1d73843527` (2,570 retrievable / 2,722 catalog chunks / 20 collections). Every staged structural release gate passed at Recall@K `1.0` and point recall `1.0` before activation.
- The manually maintained professional-book evidence sets now pass on the active production path: `误差理论与数据处理` 22/22 (MRR `0.9091`), the sensor group 13/13 (MRR `0.6923`), and `优化设计` 6/6 through production retrieval plus final EvidencePack. The optimization set covers a definition, a four-stage list, the general model formula, a worked cargo-box example, a table comparison, and a method classification.
- Fixed only evidence-preservation failures exposed by those reviewed cases: numbered phase lists retain their forward members; explicit table numbers match a table title preserved in the table block's first line; explicit worked-example labels bind only the named example and its following formula/prose siblings; formula neighborhoods survive the final support gate; Chinese topic names tolerate an intervening attributive `的`. No model-generated facts or direct edits to active index maps were used.
- Focused verification: retrieval/evidence regression files `62 passed`; active `优化设计` Context retrieval `6/6`, lifecycle `5/5`, `offline_passed=true`, and no remaining ingestion/retrieval/generation/verification failure bucket. This expansion pass did not call a paid model because its acceptance target is source-reviewed final evidence coverage; the earlier current-answer baseline remains the online generation baseline.
- Review metadata deliberately remains `human_signoff=false`: the representative cases were checked against source blocks by engineering review, not falsely presented as instructor approval. No actual考研数学 textbook source, Canonical IR, or importable PDF/DOCX was found under the repository data or artifact roots, so the overall fourth ordered item remains blocked on that input and performance work has not started.

# 2026-09-13 - Exercise to Review Learning Loop Acceptance

- Completed one packaged Electron learning cycle in the isolated acceptance data directory: source candidate review and correction, exercise import, practice start, mid-session exit and process restart, wrong-answer submission, mistake creation with cause, textbook-backed Qwen explanation, SM-2 review, final process restart, and due-queue recovery.
- The same candidate, exercise, practice session, answer/note, mistake link, wrong answer, cause, RAG explanation, review history, and next-review date survived both restarts. The reviewed item left the due queue after scheduling, and the five expected learning events were persisted without duplicate records.
- Final acceptance report contains 18/18 passing checks at `artifacts/learning_loop_acceptance_20260913/final_report.json` (SHA-256 `D457CD68793C069FAB8B3C069CA52B699177462DC26A17E9D518405E77C82A28`). This was an end-to-end acceptance run against the already packaged desktop backend; no unrelated broad test suite was run.

# 2026-09-13 - Current Real Answer Quality Baseline

- Reused the production Context v3, final-EvidencePack RAG, and online Figure harnesses. Added six manually source-reviewed `优化设计` text cases (definition, list, formula, worked example, table comparison, and method classification) plus three manually inspected Figure cases (engineering geometry, algorithm flowchart, and constraint geometry). Review metadata explicitly keeps `human_signoff=false`; this engineering review does not impersonate instructor approval.
- The current legacy production corpus is not answer-ready: Context retrieval is `0/15` while deterministic LearningTask lifecycle remains `5/5`; the sensor RAG set passes only its four out-of-scope negatives and fails all nine answerable cases; the error-theory set passes only two out-of-scope negatives and fails all twenty answerable cases. These are retrieval failures, so no paid answer calls were made for them.
- The 24-case sensor Figure set reaches `24/24` Figure-search Top-3, but every case stops before the model because the active index cannot prove schema-6 Figure provenance. The online harness now classifies `FigureIndexOutOfDateError` as ingestion rather than model failure.
- On the isolated schema-6 `优化设计` index, two of six manually reviewed text cases passed the final EvidencePack gate. Exactly those two were sent to the configured Qwen 3.7 Plus model; both covered all required answer points, used valid current-turn citations, and passed deterministic answer verification. The other four were skipped before model invocation and recorded as retrieval failures. Context Eval schema 5 now records retrieval/generation/verification buckets and never spends an online call after a failed retrieval gate.
- The three `优化设计` Figure cases made no model calls: one failed Figure search and two exposed a Canonical/index fingerprint mismatch (`a9a837...` current versus `c6ff58...` manifest). This is an ingestion/provenance failure, not a visual-model score. The consolidated baseline and SHA-256-linked raw reports are under `artifacts/quality_baseline_20260913/current_answer_quality_baseline.json`; the release gate remains failed even though the baseline itself is complete.
- Focused evaluator regressions passed: Context failure-layer/online-skip behavior `4 passed`; stale Figure-index classification `1 passed`. No broad regression suite was run.

# 2026-09-13 - Local PDF Import Fallback Repair

- Restored `PDFParser._fallback_single_chapter`, which the no-TOC/no-vision path already called but did not define.
- The fallback now preserves the complete PDF text layer and the full page range instead of truncating the source at 10,000 characters.
- Added a focused regression for the fallback contract. This unblocks first-run desktop textbook import when no model credential is configured for TOC vision.
- Corrected generated list probes to express an explicit enumeration intent, group only contiguous occurrences of repeated section headings, and retain an inline first numbered item with its same-section neighbors when a list is not a named method/formula group. This prevents fabricated cross-chapter probe requirements and keeps valid adjacent list items in EvidencePack ordering. The current 32-case staged lexical path reaches 1.0 recall, MRR, and point recall across formula/list/example/table gates.
- Packaged Electron acceptance used a fresh `userdata_v6` root. First launch reported an empty textbook list and a missing sample vector bundle while the bundled ONNX runtime was ready. Import job `82dd7e14ff9c44329142f0f183f33e5a` then imported the externally prepared `优化设计` MinerU archive and indexed 2,570 retrieval chunks from 2,722 catalog chunks across 64 chapters. The activated v6 index has 277 mapped/SQLite collections and all 32 production retrieval/EvidencePack release cases passed at 1.0 recall, MRR, and point recall.
- Forced Electron/backend termination followed by restart preserved the same book id, import-job terminal record, Canonical IR, index version `9ed872dacb504da4`, and healthy retrieval state. A verified 72,829,209-byte derived-inclusive backup (`SHA-256 935a19cce3ae7222ca1c1c3c0f5a4922c73a22e1707e8b534bf66b5211e7fb3e`) restored a deliberately changed book role/priority/group to the original values after restart without invalidating the vector index.
- The rebuilt NSIS installer is 281,996,110 bytes with SHA-256 `4264333CC19ED47982F71F13CBBEDA88CA43B3CB345D0D7FFE4792F586B61834`. It remains unsigned, and no private textbook is included because the repository does not contain redistribution evidence. Native-window screenshot capture was unavailable in the active Computer Use runtime, so this pass does not claim new visual UI evidence. After explicit paid/data-egress approval, real Qwen stop/resume completed on the same task/turn with a new run id, passed answer verification, and recovered the same two messages and task checkpoints after forced Electron restart without duplication.

# 2026-09-12 - Isolated Desktop Acceptance Data Directory

- Added an explicit `KAOYAN_USER_DATA_DIR` desktop override so packaged and development Electron acceptance runs can use a clean data root without touching the stable legacy user directory. When the variable is absent, existing installations continue to use the unchanged pre-Texa locations.
- Corrected first-run resource readiness so an automatically created empty `chroma.sqlite3` is not presented as an installed example vector bundle. Readiness now requires a non-empty scoped chapter map consistent with the SQLite snapshot.
- This is an acceptance/recovery safety boundary only. It does not migrate, delete, or copy existing user data and does not alter backend storage schemas.
- Validation is limited to the desktop runtime contract plus the current packaged first-launch, import, restart, and backup/restore acceptance path.

# 2026-09-11 - Authorized Committed-outcome Effect Recovery

- Enabled one lifecycle-owned recovery worker for explicit `execution-effect/v1` entries atomically saved with completed/degraded outcomes. Task-bound Chat graph feedback now returns proposals; synchronous, streaming and resumed image answers use the same commit-before-effect boundary. Uncommitted, interrupted, stale-run and historical tasks without explicit effects cannot authorize recovery writes.
- Added permanent operation receipts to chapter study, SM-2 review, concept exposure and concept candidates; reused unique mistake and learning-event IDs. Concept extraction/linking results are frozen before domain writes and reused after failures. Model preparation runs outside the task lock with 30-second provider timeouts and no client retries. The worker does not generate answers or execute unconfirmed pending actions.
- Retained image inputs by task in a stable managed path using atomic copy. Optional mistake import still requires the saved request selection. A successful domain write followed by a lost effect receipt can retry without duplicate records or counts. Pending-action reject reads domain receipts before changing status, and repairs already-executed actions to confirmed.
- Pending learning records preserve the saved answer and show a quiet recovery status. A read-only task endpoint supplies updates; only a completed import receipt claims success. Background projection updates the original assistant message by ID, including older messages, without appending another answer. Late terminal snapshots cannot overwrite frozen effects or receipts.
- Storage impact: additive fields in existing task/progress/SM-2/ConceptMemory JSON, with receipt retention independent of display-log trimming. No dependency change, SQLite schema migration, historical task backfill or learning-data deletion. Guarantees target the existing single managed desktop backend process; multiple writers to one data directory remain unsupported. Shutdown checks cancellation between domain steps and waits up to two seconds; an already-entered synchronous write may finish and any remaining work stays durable.
- Validation: full Python 3.10 suite 807 passed; frontend 131 passed; ESLint and production build passed; desktop runtime tests 3 passed and script checks passed. Fault injection covers every domain write boundary, duplicate recovery, frozen-result restart, blocked providers, shutdown, image-copy failures and lost pending-action receipts. Tests used isolated data and no paid provider evaluation.
- Inspected the real status component with an isolated browser fixture at 1024/1440/1920 widths, including automatic pending-to-imported transition and clean console. Native Electron screenshots were not taken; desktop verification used automated regression. Existing Starlette/httpx and Vite warnings remain. Frontend/desktop process spawning required authorized reruns outside the Windows sandbox after `spawn EPERM`.
- This user-authorized implementation supersedes the previous entry's pending effect-recovery boundary. See `docs/execution-effects-recovery-proposal.md` for the enabled behavior and limits. No Git commit or push was performed.

# 2026-09-10 - Execution Run Model and Backend-owned Mistake Messages

- Added one backend run transition function shared by the emitter and persisted sequence validator. It checks full request/task/run/conversation/turn identity, monotonic sequence, terminal status, and closure after an input gate or terminal. A failed persistence callback no longer consumes the emitter sequence or closes it. Removed the redundant emitter terminal flag.
- Online frontend consumers now keep one run per HTTP execution; an unsolicited run change cannot grant authority. Preflight-to-task binding resets its sequence boundary. History replay first selects the task's persisted `active_run_id`, so an old terminal cannot hide a later resume. Sparse milestones preserve the authoritative SQLite answer instead of rebuilding text from unavailable token history.
- Task-bound Mistake answers, errors and input gates now atomically store message projections with the task boundary. The backend projects the user and assistant messages using the existing turn/role identity; completing an input-gated task replaces the waiting assistant in place. The frontend consumes the backend message ID and no longer writes that exchange again. Taskless cached-mistake explanations retain their existing client persistence behavior.
- Added bounded model exploration (1,296 backend action sequences and 625 frontend sequences), identity-drift and failed-emitter-persistence regressions, active-run replay checks, and real SQLite tests for lost Mistake projection/recovery and input-gate completion without duplicate assistant messages.
- Validation: full Python suite 769 passed; frontend 128 passed; ESLint and production build passed. Existing Starlette/httpx and Vite chunk/mixed-import/plugin-timing warnings remain. Python tests used isolated temporary data and no paid provider evaluation.
- Domain side effects are not claimed complete: automatic approval rejected the proposed background effect-recovery worker. No such worker is installed. Direct optional Mistake import, graph feedback ownership and the reject-after-lost-confirmation-receipt case remain pending; the concrete scope and acceptance tests are in `docs/execution-effects-recovery-proposal.md`.

# 2026-09-09 - P1 Execution Ownership and Crash Recovery

- Fenced Chat stream/non-stream, Figure and task-bound Mistake completion by the persisted active run. Ordinary save/checkpoint cannot change run ownership; resume claims the new run atomically. Stop requests now require the current `run_id`; the desktop frontend sends it. Clarification continuation rebinds the task to the new turn before emitting events.
- Added an atomic LearningTask outcome record containing the task status, terminal event and pending message projections. Chat and Figure project messages from this record using the existing SQLite turn/role idempotency key. Startup interrupts orphaned running tasks and retries pending projections; resume repairs a user message missing after a crash between task creation and the first message write. No SQLite schema migration or user-data deletion is required. Existing task JSON remains readable; old binaries do not implement the new recovery contract.
- Streaming responses explicitly own and close their producers, including ASGI send failure and cancellation during preflight. Provider queues are capped at 32 entries and stream workers at four. Cancellation fences the task immediately; an arbitrary synchronous provider blocked in `next()` can retain its worker until it returns or times out. This is bounded resource retention, not forced socket cancellation or exactly-once network delivery.
- Filtered thinking incrementally before LangGraph token delivery. Final-output filtering remains in place.
- Added interrupted visual-task resume and run-bound interruption. Resume reuses saved Visual IR and the selected exact/method-only policy, retries unfinished vision work, and restores the input gate if required materials remain missing. The frontend routes visual tasks back to the visual producer instead of the Chat producer.
- Coordinated active-index publication with complete retrieval reads in the managed backend process; different readers can still run concurrently. Reject mixed per-book evidence versions before fusion and at the final projection, rehydrate KG occurrences from the matching lexical snapshot, and validate every book represented in a resume checkpoint. This lock does not coordinate separate backend processes.
- Upgraded the derived Session Ledger contract to v3: its freshness marker is the conversation event revision, including partial-message completion and scope changes. Stale resolver writes fail the revision check and rebuild from SQLite. Ledger and conversation mutations share the conversation lock, avoiding reversed lock acquisition during reclassification/splitting. Existing v2 ledgers rebuild from authoritative messages.
- Added stable operation receipts for confirmed mistake insertion, concept review and practice-session creation. A retry after the domain write succeeds but the pending-action receipt fails returns the same operation instead of repeating it. Concept review receipts are stored with the concept update; practice-session replacement and creation share one SQLite transaction.

## Validation

- Final full Python regression: 760 passed, with the existing Starlette/httpx deprecation warning. Tests use Python 3.10 and isolated temporary data/configuration; no paid provider evaluation was run.
- Added fault-injection and concurrency regressions for atomic outcome failure, SQLite-write/receipt crashes, recovery replay, delayed preflight ownership, stale non-stream answers, ASGI 2.3/2.4 disconnects, bounded blocked providers, missing user-message recovery, visual resume/input gates, ledger revision races and lock ordering, index publication/read overlap, and confirmation retry after a lost receipt.
- Frontend: 125 tests passed; ESLint and the production build passed. Electron runtime regression: 3 passed. Vite required the same command outside the Windows sandbox after `spawn EPERM`; existing MathLive chunk-size, MarkdownRenderer mixed-import and plugin-timing warnings remain. No manual Electron UI inspection was performed in this repair pass.

## Remaining boundaries

- This is a bounded P1 repair, not completion of the entire architecture audit. The typed retrieval pipeline migration and a unified model for all ExecutionEvent/replay/input-gate sequences remain separate work.
- Task-bound Mistake terminals now commit with task status, but its conversation projection still follows the existing client persistence path. Its direct optional mistake import and graph background feedback also need a broader domain-write ownership pass; this change does not claim crash-atomicity for those side effects.
- Pending-action confirmation retries are idempotent for the three supported operations; reconciling a reject after an already-applied effect loses its receipt remains outside this patch. Pending outcome projection failures are retried at startup or before a resumable task acquires its next run.
