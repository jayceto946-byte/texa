"""Generate the evidence-backed ONNX Phase 2 release feasibility report."""
...
def release_paths(variant: str) -> dict[str, Path]:
    release = PHASE / variant / "release"
    unpacked = release / "win-unpacked"
    return {
        "release": release,
        "installer": next(release.glob("*.exe")),
        "zip": next(release.glob("*.zip")),
        "unpacked": unpacked,
        "backend": unpacked / "resources" / "backend",
        "internal": unpacked / "resources" / "backend" / "backend_server" / "_internal",
    }


def top_components(paths: dict[str, Path], limit: int = 20) -> list[dict]:
    internal = paths["internal"]
    values: list[tuple[str, int]] = []
    for item in internal.iterdir():
        if item.name == "sample_data" and item.is_dir():
            for child in item.iterdir():
                values.append((f"backend/_internal/sample_data/{child.name}", size(child)))
        else:
            values.append((f"backend/_internal/{item.name}", size(item)))
    unpacked = paths["unpacked"]
    for item in unpacked.iterdir():
        if item.name != "resources":
            values.append((item.name, size(item)))
    resources = unpacked / "resources"
    for item in resources.iterdir():
        if item.name != "backend":
            values.append((f"resources/{item.name}", size(item)))
    values.sort(key=lambda item: item[1], reverse=True)
    return [{"path": name, "bytes": value, "mib": round(value / 1024 / 1024, 3)} for name, value in values[:limit]]


def retrieval_metrics(torch: dict, onnx: dict) -> dict:
    fixture = json.loads((ROOT / "evaluation" / "datasets" / "embedding_retrieval.json").read_text(encoding="utf-8"))
    expected = {item["id"]: set(item.get("expected_chunk_ids", [])) for item in fixture["queries"]}
    trows = {row["id"]: row for row in torch["rows"]}
    orows = {row["id"]: row for row in onnx["rows"]}
    ...
    for k in (1, 3, 5, 10):
        ...
        recall_t = 0
        recall_o = 0
        for query_id in sorted(trows):
            tids = trows[query_id]["ids"][:k]
            oids = orows[query_id]["ids"][:k]
            overlaps.append(len(set(tids) & set(oids)) / k)
            exact += tids == oids
            labels = expected.get(query_id, set())
            recall_t += bool(labels & set(tids))
            recall_o += bool(labels & set(oids))
        count = len(overlaps)
        result[str(k)] = {
            "mean_set_overlap": sum(overlaps) / count,
            "identical_order_queries": exact,
            "torch_recall": recall_t / count,
            "onnx_recall": recall_o / count,
        }
    return result
...
def main() -> None:
    torch = load("retrieval_torch.json")
    onnx = load("retrieval_onnx.json")
    runtime = load("runtime_real_index.json")
    startup = load("packaged_startup.json")
    packaged = load("packaged_embedding_ingestion.json")
    failures = load("failure_handling.json")
    retrieval = retrieval_metrics(torch, onnx)
    baseline = release_paths("baseline")
    candidate = release_paths("candidate")

    sizes = {
        "baseline": {key: size(baseline[key]) for key in ("installer", "zip", "unpacked", "backend")},
        "candidate": {key: size(candidate[key]) for key in ("installer", "zip", "unpacked", "backend")},
    }
    reductions = {
        key: {
            "bytes": sizes["baseline"][key] - sizes["candidate"][key],
            "fraction": (sizes["baseline"][key] - sizes["candidate"][key]) / sizes["baseline"][key],
        }
        for key in sizes["baseline"]
    }
    onnx_model = next(candidate["internal"].glob("sample_data/models/**/bge-small-zh-v1.5-fp32.onnx"))
    ort_bytes = size(candidate["internal"] / "onnxruntime")
    new_onnx_bytes = onnx_model.stat().st_size + ort_bytes
    gross_removed = reductions["backend"]["bytes"] + new_onnx_bytes
    forbidden = []
    forbidden_names = {"torch", "sentence_transformers", "transformers", "safetensors"}
    for item in candidate["backend"].rglob("*"):
        lowered = {part.lower() for part in item.relative_to(candidate["backend"]).parts}
        if lowered & forbidden_names:
            forbidden.append(str(item.relative_to(candidate["backend"])))

    interactive_t = packaged["interactive"]["baseline"]["cases"]["1"]
    interactive_o = packaged["interactive"]["candidate"]["cases"]["1"]
    ingestion_t = packaged["ingestion"]["baseline"]["result"]
    ingestion_o = packaged["ingestion"]["candidate"]["result"]
    ...
    size_rows = []
    labels = {"installer": "NSIS installer", "zip": "portable ZIP", "unpacked": "win-unpacked / installed", "backend": "backend runtime"}
    for key in ("installer", "zip", "unpacked", "backend"):
        delta = reductions[key]["bytes"]
        size_rows.append([labels[key], mib(sizes["baseline"][key]), mib(sizes["candidate"][key]), mib(delta), pct(delta, sizes["baseline"][key])])

    retrieval_rows = []
    for k in (1, 3, 5, 10):
        item = retrieval[str(k)]
        retrieval_rows.append([f"Top-{k}", f"{item['mean_set_overlap']:.3%}", f"{item['identical_order_queries']}/100", f"{item['torch_recall']:.0%}", f"{item['onnx_recall']:.0%}"])

    top_baseline = top_components(baseline)
    top_candidate = top_components(candidate)
    top_rows = []
    for index in range(20):
        left = top_baseline[index] if index < len(top_baseline) else {"path": "", "bytes": 0}
        right = top_candidate[index] if index < len(top_candidate) else {"path": "", "bytes": 0}
        top_rows.append([str(index + 1), left["path"], mib(left["bytes"]), right["path"], mib(right["bytes"])])

    failure_rows = [[item["case"], item["code"], item["status"], "有" if item.get("diagnostic_log_present") else "分类器验证", "Repair Texa runtime/model and retry"] for item in failures["cases"]]

    report = f"""# ONNX Runtime Phase 2 — Dependency, Reachability & Windows Release Feasibility
...
- 真正 Windows package 的 installed/unpacked 净减少 **{mib(reductions['unpacked']['bytes'])} ({reductions['unpacked']['fraction']:.2%})**；installer 减少 **{mib(reductions['installer']['bytes'])} ({reductions['installer']['fraction']:.2%})**。installed size 通过 400 MB gate；所有口径均通过 25% relative gate；600 MB STRONG PASS 未达到。
- Packaged batch=1 median：Torch {interactive_t['median_ms']:.2f} ms，ONNX {interactive_o['median_ms']:.2f} ms。500 texts：Torch {ingestion_t['texts_per_second']:.2f} texts/s，ONNX {ingestion_o['texts_per_second']:.2f} texts/s；Candidate 为 Torch 的 {ingestion_o['texts_per_second']/ingestion_t['texts_per_second']:.1%}，远高于 90% gate。
...
{table(inventory_rows, ['类', 'file', 'symbol/import', '用途', '调用入口', 'production reachable', '进入 release', '普通用户需要', 'Torch-free 处理'])}
...
| 检查 | 结果 |
| --- | --- |
| backend import | PASS（{runtime['backend_import']}） |
| FastAPI startup / `/health` | PASS / `{runtime['health_status']}` |
| embedding load / single query | PASS，{runtime['embedding_dimensions']} dims，{runtime['embedding_query_ms']:.2f} ms（source env spot） |
| Chroma load | PASS，{runtime['collection_count']} collections |
| retrieval / textbook path | PASS，{runtime['textbook_evidence_count']} evidence |
| generic QA | PASS，`{runtime['generic_retrieval_status']}` |
| book/index discovery | PASS，{runtime['book_count']} books |
| normal warmup | `{runtime['warmup']['status']}`，error=`{runtime['warmup']['error']}` |
...
{table(retrieval_rows, ['K', 'Torch/ONNX mean set overlap', '完全同序 query', 'Torch relevance recall', 'ONNX relevance recall'])}

Torch mean retrieval {torch['retrieval_mean_ms']:.2f} ms；ONNX {onnx['retrieval_mean_ms']:.2f} ms。两次均复现同一个既有“第八章 热电式传感器”HNSW segment `Nothing found on disk`，不是 ONNX 新问题。结合 Phase 1 cosine 1.0 和 isolated retrieval parity 100%，剩余差异可归因于 HNSW approximation、极小浮点/tie ordering 与 hybrid fan-out，而不是向量语义改变。**已有教材无需重新向量化。**
...
两套都是真实 PyInstaller backend + electron-builder Windows NSIS/ZIP/win-unpacked；Candidate backend 路径扫描命中 forbidden packages 数量：**{len(forbidden)}**。

{table(size_rows, ['口径', 'Baseline', 'Candidate', '净减少', '减少比例'])}

Size Gate：installed/unpacked 和 backend 的绝对减少均为 {mib(reductions['unpacked']['bytes'])}，**≥400 MB PASS**；未达 ≥600 MB STRONG PASS。压缩 installer/ZIP 绝对值不到 400 MB，但分别减少 {reductions['installer']['fraction']:.1%}/{reductions['zip']['fraction']:.1%}，均通过 ≥25% significant gate。
...
Backend 口径估算：gross removed bytes = **{mib(gross_removed)}**；new ONNX bytes（model + ORT Python/native runtime）= **{mib(new_onnx_bytes)}**；net release/backend reduction = **{mib(reductions['backend']['bytes'])}**。模型 ONNX 本体 {mib(onnx_model.stat().st_size)}，ORT package/native {mib(ort_bytes)}。

{table(top_rows, ['#', 'Baseline largest component', 'size', 'Candidate largest component', 'size'])}
...
| --- | --- | --- | --- |
| first health | {startup['baseline']['first_health_ms']['median']:.0f} / {startup['baseline']['first_health_ms']['p95']:.0f} ms | {startup['candidate']['first_health_ms']['median']:.0f} / {startup['candidate']['first_health_ms']['p95']:.0f} ms | Candidate entrypoint 在启动 uvicorn 前同步验证 ONNX，因此 health 稍晚 |
| embedding + Chroma ready | {startup['baseline']['warmup_ready_ms']['median']:.0f} / {startup['baseline']['warmup_ready_ms']['p95']:.0f} ms | {startup['candidate']['warmup_ready_ms']['median']:.0f} / {startup['candidate']['warmup_ready_ms']['p95']:.0f} ms | Candidate full-ready median 快约 {1-startup['candidate']['warmup_ready_ms']['median']/startup['baseline']['warmup_ready_ms']['median']:.1%} |
| first retrieval wall | {startup['baseline']['retrieval_wall_ms']['median']:.0f} / {startup['baseline']['retrieval_wall_ms']['p95']:.0f} ms | {startup['candidate']['retrieval_wall_ms']['median']:.0f} / {startup['candidate']['retrieval_wall_ms']['p95']:.0f} ms | Candidate 不差于 Torch |
...
| --- | ---: | ---: | ---: |
| Torch/ST | {interactive_t['median_ms']:.3f} ms | {interactive_t['p95_ms']:.3f} ms | {mib(interactive_t['peak_rss_bytes_p95'])} |
| ONNX | {interactive_o['median_ms']:.3f} ms | {interactive_o['p95_ms']:.3f} ms | {mib(interactive_o['peak_rss_bytes_p95'])} |

真实 packaged backend 的 retrieval stage median 同样从 {startup['baseline']['retrieval_stage_ms']['median']:.2f} ms 降到 {startup['candidate']['retrieval_stage_ms']['median']:.2f} ms。
...
| --- | ---: | ---: | ---: | ---: |
| Torch/ST | {ingestion_t['median_ms']/1000:.2f} s | {ingestion_t['p95_ms']/1000:.2f} s | {ingestion_t['texts_per_second']:.2f} texts/s | {mib(ingestion_t['peak_rss_bytes_p95'])} |
| ONNX | {ingestion_o['median_ms']/1000:.2f} s | {ingestion_o['p95_ms']/1000:.2f} s | {ingestion_o['texts_per_second']:.2f} texts/s | {mib(ingestion_o['peak_rss_bytes_p95'])} |

ONNX throughput = Torch 的 {ingestion_o['texts_per_second']/ingestion_t['texts_per_second']:.1%}；packaged gate `>= Torch 90%` 明确通过，RSS 也未回归。

## 17. Failure Handling

实验 Candidate 不 silent fallback Torch。每个场景在 fresh Torch-free process 验证：

{table(failure_rows, ['场景', 'structured code', '结果', 'diagnostic log', '用户消息'])}
...
Candidate 不安装/不打包 torch 与 sentence-transformers（同时也无 transformers/safetensors），core runtime、packaged backend 和真实 Electron UI 工作。installed/backend 均净减约 {mib(reductions['backend']['bytes'])}；installed 减少 {reductions['unpacked']['fraction']:.1%}，backend 减少 {reductions['backend']['fraction']:.1%}，通过 400 MB + 25% gate；compressed artifacts 仅通过 relative gate，未达 400 MB absolute。
...
"""
    ...
    machine = {
        "verdicts": {
            "onnx_embedding_production_backend": "GO",
            "torch_free_standard_release": "GO",
            "optional_crossencoder_strategy": "MOVE TO OPTIONAL",
            "production_torch_fallback": "REMOVE",
        },
        "sizes": sizes,
        "reductions": reductions,
        "new_onnx_bytes": new_onnx_bytes,
        "gross_removed_bytes_estimate": gross_removed,
        "candidate_forbidden_paths": forbidden,
        "retrieval": retrieval,
        "top20": {"baseline": top_baseline, "candidate": top_candidate},
        "startup": startup,
        "packaged_embedding": packaged,
        "failure_handling": failures,
    }
    (PHASE / "phase2.json").write_text(json.dumps(machine, ensure_ascii=False, indent=2), encoding="utf-8")
    (PHASE / "report.md").write_text(report, encoding="utf-8")
    print(json.dumps({"status": "PASS", "report": str(PHASE / "report.md"), "verdicts": machine["verdicts"]}, ensure_ascii=False))
